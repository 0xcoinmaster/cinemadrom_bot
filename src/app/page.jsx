...
export default function Home() {
  ...
  useEffect(() => {
    DefaultIcons.map((item) => {
      if (item.path == router) setCurrentIndex(item.id);
    });
  }, [router]);
  ...
  return (
    <div className="relative w-full max-w-3xl mx-auto overflow-hidden relative">
      {captcha == true ? (
        <div
          className={`flex transition-transform ${currentIndex == 0 || currentIndex == 1 || currentIndex == 7 || currentIndex == 8 ? 'duration-0' : 'duration-500'}`}
