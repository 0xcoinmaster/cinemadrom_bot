...
const Main = () => {
  ...
  useEffect(() => {
    if (state == 'success') setMining('Claim');
  }, [state]);
    ...
    <main className="flex flex-col justify-center items-center max-w-[432px] mx-auto my-0 text-white py-[5px] mt-[30px]">
        ...
        <div className="flex flex-col justify-center items-center w-full h-full bg-gradient-to-r from-[#585977] to-[#272735] rounded-[15px]">
          <div className="flex justify-between items-start w-full px-2">
            ...
            <div className="flex items-center gap-4">
              <button
                className="bg-[#D9D9D9] border-[#F5A4A4] border-2 p-[5px] rounded-[9px]"
                onClick={() => setRouter('/rules')}
              >
                <Image src="/Rules.png" width={35} height={35} alt="Rules" />
              </button>
              <button
                className="bg-[#D9D9D9] border-[#A9DD3C] border-2 p-[5px] rounded-[9px]"
                onClick={() => setRouter('/account')}
              >
                <Image src="/Vector.png" width={37} height={37} alt="Vector" />
              </button>
            </div>
          </div>
          <p className="flex items-start text-[15px] w-full font-medium pl-[57px]">
            Total <span className="text-[#E974F3] px-2">MVH</span> mining
            Balance:
          </p>
          <div className="flex justify-between items-center w-full px-3">
            ...
            <p className="bg-[#F7C08E] border-[#CB7334] border-2 px-[12px] py-[11px] rounded-[15px] text-black text-[17px] font-medium">
              Withdraw
            </p>
          </div>
        </div>
      ...
      <section className="w-[380px] h-[186px] relative grid mb-2">
        <div className="flex flex-col justify-between items-center z-30 pt-[15px] pb-[10px]">
          <p className="text-[#F9D61B] text-[18px] font-medium">
            100 % Filled in 1 hours
          </p>
          ...
          <p className="font-light text-[15px]">
            0.01 <span className="text-[#E974F3]">MVH</span> / 1 hour
          </p>
          <div className="flex justify-between w-full px-2">
            <button
              className="w-[85px] h-[35px] bg-[#F7C08E] border-[#CB7334] border-2 rounded-[15px] font-medium text-[18px] text-black"
              onClick={() => setRouter('/earn')}
            >
              Earn
            </button>

            <button
              className="w-[150px] h-[35px] bg-[#F7C08E] border-[#CB7334] border-2 rounded-[15px] font-medium text-[18px] text-black"
              onClick={() => startMining()}
            >
              {mining}
            </button>
